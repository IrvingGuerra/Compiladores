...
Inst   progs[NPROG];   /* la Máquina */
Inst   *progp;        /* siguiente sitio libre para la generación de
 código */
Inst   *progp1;        
Inst   *pc;              /* contador de programa durante la ejecución */
Inst   *progbase = prog; /* Inicie del subprograaa actual */
Inst   *progbase1 = progs;
...
void execute(Inst *p){
   Simbolo *s;
   Inst *pc1;
   for  (pc1 = progs; strcmp(*pc1, "STOP") ; ) {
         //printf("000cta = (%d, %s, %d)\n", pc-p , *pc, pc-prog);
         printf("000cta = (%d) (%s)\n" , pc1-progs, *pc1);
         pc1 = pc1 + 1; 
   }
   if(!strcmp(*pc1, "STOP"))
            printf("SALI execute IFIF = ( %d ) pc=(%s) \n", pc1-progs, *pc1 );
   printf("entre  execute= ( %d )\n", p-prog);
   //for  (pc = p ; strcmp(*pc, "STOP") && !returning ; ) {//faltaba && !returning
        //puts(*pc);
        //printf("cta1 = ( %d ) pc=(%s) \n", pc-p, *pc );
        //debug printf("cta = ( %d ) pc=(%s) \n", pc-prog, *pc );
